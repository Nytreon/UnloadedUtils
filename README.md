...
| `getChunkState()` | `dimension`, `pos` | `1 or 0 or -1` | Returns `1` (Loaded), `0` (Unloaded), or `-1` (Invalid). |
...
- **Fast ticking area**: turns out mojang sucks at promise returning and you can read/write to chunks much earlier than the returned promise, so by ignoring the promise and using a wait and check every tick we can get it much sooner so I wrapped it in my own promise to ensure ease of use.
